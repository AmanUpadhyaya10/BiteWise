    ...
    try {
      ...
      if (result.mode === "single") {
        ...
        setDetectedFoods([{
          ...
          top_k: result.top_k,  // ← pass all top-5
        }]);
        ...
        setDetectedFoods(result.items.map((item) => ({
          ...
          top_k: item.top_k,  // ← pass all top-5 per item
        })));
      }
    } catch (err: unknown) {
        ...
        <div className="flex-1 flex flex-col px-5">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-400">
                            {food.nutrition?.kcal != null ? `~${Math.round(food.nutrition.kcal)} kcal/100g` : "—"}
                          </p>
                        </div>
                      </div>
          ...
          {!detectedFoods.length && (
            <div className="flex flex-col items-center gap-4 py-5">
              <button onClick={() => fileRef.current?.click()} disabled={isScanning}
